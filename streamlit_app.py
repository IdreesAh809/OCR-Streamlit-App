...
# initialize Tesseract and session variables
...
with col_upload_2:
    ...
    if st.session_state.uploaded_file and st.session_state.image is not None:
        st.markdown("---")
    ...
    with col1:
        if st.session_state.preview_processed_image:
            ...
            if st.session_state.image is not None:
                image = opencv.convert_to_rgb(st.session_state.image)
            ...
            if st.session_state.raw_image is not None:
                raw_image = opencv.convert_to_rgb(st.session_state.raw_image)
